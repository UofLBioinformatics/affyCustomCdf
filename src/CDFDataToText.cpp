...
CDFDataToTextFile::CDFDataToTextFile()
{
  m_cb = NULL;
  m_pfileStream = NULL;
}
...
bool CDFDataToTextFile::isOpen() {
  return ((m_pfileStream != NULL) && (m_pfileStream->is_open()));
}

void CDFDataToTextFile::close(void)
{
  if (isOpen()) {
      m_pfileStream->close();
      delete m_pfileStream;
...
bool CDFDataToTextFile::open(const AffxString& strFileName )
{
...
void CDFDataToTextFile::write(const char *psz) {
  if (m_pfileStream != NULL) {
    m_pfileStream->write(psz, (std::streamsize)strlen(psz));
  }
}

void CDFDataToTextFile::writeLine(const AffxString& str)
{
 ...
 * @param strMessage - The error message to report.
 */
void CDFDataToText::error(const AffxString& strMessage)
{
	Err::errAbort(strMessage);
}

void CDFDataToText::OutputQCUnits(){

	int numQCUnitsInFile  = cdf->GetHeader().GetNumQCProbeSets();

	affxcdf::CCDFQCProbeSetInformation qcunit;
	affxcdf::CCDFQCProbeInformation qcprobe;
...
void CDFDataToText::CreateCDFFile(const std::string& strFileNameOut, int _maxUnit, bool bHeader, bool bBody)
{
  try 	{
    ...
    //if (cdf->Read() != false) {
      ...
      if (m_file.open(m_strFileNameOut) ){
        ...
        Err::errAbort("A problem occurred while processing file (Cannot open output file) ");
      }
    //}
    //else
 ...
 */
void CDFDataToText::OutputHeader()
{
 ...
 */
void CDFDataToText::OutputGroupInfo(GenericData &gdata, const std::string &groupName, int groupIndex)
{

	//Get the probe set information
	affxcdf::CCDFProbeSetInformation set;
	...
	m_file.writeLine("[Unit" + ::getInt((int)set.GetProbeSetNumber()) + "]");
	...
	m_file.writeLine("UnitNumber=" + ::getInt((int)set.GetProbeSetNumber()));
	int UnitType = -1;
	...
	{
		affxcdf::CCDFProbeGroupInformation group;
		...
		{
			affxcdf::CCDFProbeInformation probe;
			group.GetCell(icell,probe);

			int index = probe.GetY() * numCols + probe.GetX();

			m_file.writeLine("Cell" + ::getInt((int)(icell+1)) + "=" + ::getInt((int)probe.GetX()) + "\t" + ::getInt((int)probe.GetY()) \
 ...
 */
void CDFDataToText::OutputGroupsAndSets(GenericData &gdata)
{
