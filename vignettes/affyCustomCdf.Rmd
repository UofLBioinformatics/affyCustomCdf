...
author:
- Ernur Saka^[University of Louisville]
...
must be tab delimited without a header line. Columns are in order: x location of a
probe, y location of a probe, sense/antisense of a probe (-/+), chromosome name 
and start point. The following demonstrates the first 10 lines of the Rat 230 2.0 
probe mapping file. The file is included inside the tests directory of the 
affyCustomCdf tool. 
```html
335	337	-	8	22161426  
99	499	-	8	22161407  
...
The DNA sequences for perfect match (PM) probes is obtained from the Affymetrix&reg;
Netaffx^TM^ web site in a FASTA file format. PM probes are aligned to the genome using
Bowtie version 1.0.1 [@RN159] with the parameters -v 0 and -m 1 which returns the 
alignment results for the probes that aligned to one genomic location with 100% 
identity. Note that Bowtie version 1 is best at aligning shorter sequences 
(25-50 bp) as found with microarray probes while the most resent versions of 
bowtie are optimized for long sequence reads (>50 bp). Also, the --suppress 
parameter can be used to eliminate some output for clarity. The following command 
line is an example for aligning Rat 230 2.0 PM to Rat assembly rn6. It suppresses 
the 5th, 6th and 7th default Bowtie outputs.  
```html
...
from one of the online repositories such as Ensembl[@RN92] and NCBI and indexes
can be created from DNA sequences via Bowtie-Build version 1.0.1 with default 
parameters. The following is the one line of the mapping result file.  
```html
Rat230_2:1367453_at; 335; 337; 1100; Antisense;	-	8	22161426
```
The file can be formatted via a R script to obtain the proper column arrangement. 
More script examples can be found under the tests folder with the file name 
ProbeFileClear.R
```html
fileName = "RatRn6Chr7-24Aligned.txt"
probes = read.csv2(fileName, header = FALSE, sep = "\t", quote = "\"",
dec = ",", fill = TRUE, col.names = c("names","sense","chromosome", "start","empty")  
...
write.table(probeTable, "rat.txt", quote = FALSE, row.names = FALSE,  
col.names = FALSE, sep="\t") 
remove(probeTable,X,Y,probes)
```
## 1.2 Obtaining annotation file
The affyCustomCdf tool accepts annotations as a General/Gene Transfer Format (GTF).
In the GTF file, fields must be tab delimited. The GTF format consist of nine 
columns of data per feature. The columns are seqname, source, feature, start, end, 
score, strand, frame and attribute. Comment lines in the file must start with #.
The feature column is being used to classify probes for gene features. Therefore 
region features must contain transcript, EXON, CDS and UTR key words (not case 
sensitive). If separation between 3\` UTR and 5\` UTR is desired, features must 
include the direction information such as three_prime_utr or five_prime_utr. 
Otherwise annotation will be performed without classification of 3\` and 5\`. For 
more information please check the Ensembl GFF/GTF file format definition. 
<http://www.ensembl.org/info/website/upload/gff.html>. A sample GTF file can be 
found in the tests folder.

## 1.3 Obtaining original CDF
To obtain probe information of a chip, affyCustomCdf tool uses the original
CDF file provided by Affymetrix&reg;. A specific CDF file can be obtained from 
the Affymetrix&reg; Netafffx^TM^ web site or from the Gene expression 
omnibus(GEO) [@RN117]. 
...
technology. It can be obtained from Affymetrix\textsuperscript{\textregistered}
NetAffx^TM^ [@RN197] web site.
```html
...
* **gtfFileName:** String type. The General/Gene Transfer Format(GTF) file name.
...
```
* **reportFile:**  String type. Name of the report file name. If the user does not 
provide a name, a name will be created based on the template provided in the 
flowing line.
```html
report_orginalCdfName.txt
```
* **controlProbeSetNumber = 0:** Number type. The number of control probe sets in 
the original CDF. Control probe sets were being designed to check the quality of
the experiment. They can be detected in the CDF based on the probe set names 
which usually starts with AFFX prefix such as AFFX_ratb2/X14115_at. If the number 
of control probe set is given, they will be included in the custom CDF without 
changes otherwise they will not be included. The default value is 0 (not included).
...
(opposite direction). The default value is 1 (Same direction).
    ...
    + When SD is 0, direction will not be considered during probes to annotations
    mapping. 
...
Options are regionG, gene, transcript. Default value is regionG.  
    ...
    + regionG: When regionG option is selected, probe sets are designed to target a 
specific region (exon, UTR) of a gene and consist of probes which map to the same
region of a gene.
    + gene: When regionG option is selected, probe sets are designed to target genes
...
createAffyCustomCdf("Rat230_2.cdf","rat.txt", 
"Rattus_norvegicus.Rnor_6.0.85.gtf",
...
specific annotations of interests. It can be used for data analyses of Affymetrix&reg; 
GeneChip&reg; data.

### 4.1.1 Data analysis via Custom CDFs in Windows Systems
1) Make custom CDF package and install the package
```html
cdfFileName = "RAT230_2_regionG_min_1_D_0.cdf"
cdfFilePath = "C:/Users/Ernur/Documents/MicroArrayProject/R/CreateCDF/analyze"
specy = "Rat"
pkgpath = tempdir()
...
```
3) Write a R script to analyze data produced via the Affymetrix\textsuperscript{\textregistered}
GeneChip\textsuperscript{\textregistered} of customized CDF. The following script 
...
path = "C:/Users/Ernur/Documents/MicroArrayProject/R/analyze/data"
cels2 <- list.files("C:/Users/Ernur/Documents/MicroArrayProject/R/analyze/data"
, pattern = "CEL")
rawData2 = ReadAffy(verbose=TRUE, filenames=cels2, celfile.path=path,
cdfname=cdf)
datafile = "C:/Users/Ernur/Documents/MicroArrayProject/R/analyze/data
...
normalizedData.filtered = featureFilter(normalizedData, require.entrez=FALSE,
              ...
              require.GOMF=FALSE, require.CytoBand=FALSE,remove.dupEntrez=FALSE,
...
sort.by='P')
write.table(arrangedD1 ,file="C7vsC0BrainV20-7-28.xls",row.names=T, sep="\t",
col.names=NA)
write.table(arrangedD4 ,file="C14vsC0BrainV20-7-28.xls",row.names=T, sep="\t",
col.names=NA)
```
...
Report file consists of multiple information. They are:
    ...
    1) Parameter values sent to the CreateAffyCustomCdf function.
    2) Distribution of number of probes per probe set table. 
    3) The histogram of the number of probes per probe set. 

## 4.3 Missing Probes File
In some cases FASTA file of probes may include some probes that is not used in 
the original CDF. These probes are detected and placed into a text file to inform
users. Name of the file starts with original CDF name with postfix missingProbes. i.e 
