...
setMethod("UniqueProbeRegionNoShare", signature("affyCustomCdf"),
  ...
  #Unique to 3-Utr
  threeLeft = threeUtrTotals[!(threeUtrTotals$values  %in% fiveUtrTotals$values)]
  ...
  if(dim(fiveUtrTotals)[1] > 0){
    ...
    #probeIndexses = unlist(firstIndexes, use.names = FALSE)
    #modifiedTable = rbindlist(list(modifiedTable, as.list(data.table(keys=UtrTotals$keys[probeIndexses],values=UtrTotals$values[probeIndexses]))))
  ...
  #make probes unique in each region such as one region of two different genes can't share probe.
