# Reads ensemble formated Gtf into data table which will be used to annotate
# probes.
#
# Author: Ernur Saka
###############################################################################
...
setMethod("ReadGeneStructure", signature("affyCustomCdf"),
        ...
        }else if(.object@type == "transcript"){ #Left it as else if since we
          #may add the exon based gtf option
       ...
       geneId = vector("list", length(resf))
        ...
        if(.object@type == "regionG" || .object@type == "gene"){
          ...
          id = lapply(1:length(resf),function(i) resf[[i]][[1]][1])
          id = lapply(1:length(id),function(i)
            str_replace(id[[i]],"gene_id ",""))
          ...
          geneId = lapply(1:length(resf),function(i) resf[[i]][[1]][1])
          geneId = lapply(1:length(geneId),function(i)
            str_replace(geneId[[i]],"gene_id ",""))
          # id = lapply(1:length(res),function(i) res[[i]][[1]][3])
          # id = lapply(1:length(id),function(i)
          #   str_replace(res[[i]][[1]][3]," transcript_id ",""))
          id = lapply(1:length(resf),function(i) resf[[i]][3])
          id = lapply(1:length(id),function(i)
            str_replace(resf[[i]][3]," transcript_id ",""))
