...
#'
#' @author Ernur Saka
#' @param originalCdfName String The original CDF file of the selected
...
createAffyCustomCdf = function(orginalCdfName, probeAlignmentFile, gtfFileName,
                               ...
                               minProbeSetNumber = 3, probeLength=25, SD=1,
                               type="regionG",transcriptShare = FALSE,
                               junction = FALSE, uniqueProbe=FALSE,probeShare=1)
  {
  ...
  missiggProbesFile = str_replace(toupper(basename(orginalCdfName)),".CDF","")
  missiggProbesFile = paste("missingProbes",missiggProbesFile,type,sep = "_")
  missiggProbesFile = paste(missiggProbesFile,".txt",sep = "")
  ...
  cat("Reading probes from the given txt file\n")
  probes = ReadProbes(newCdf)

  cat("Reading GTF file. It may take several minutes!\n")
  gtf = ReadGeneStructure(newCdf)

  cat("Probes are being annotated\n")
  if (type == "regionG"){
    ...
    if (uniqueProbe == FALSE) {
     ...
     if (SD ==  0){
       if (junction == FALSE)
         #!!!!!!NEW CODE WRITTTEN BUT NOT CHECKED IN THE RESULT CDF FILE COMPARE
         #FILES
    ...
    stop("type argument is not correct ", type)
  }
  ...
  #cat("Custom CDF is being created\n")
  newCdf = WriteASCIICdf(newCdf,reportFile,missiggProbesFile)
  #return(newCdf)
